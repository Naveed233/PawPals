...
export default function Events() {
  ...
  return (
    <Screen
      title="イベント・ミートアップ"
      subtitle={`${list.length}件の予定`}
      right={
        <Pressable
          onPress={() => router.push('/create-event')}
          accessibilityRole="button"
          accessibilityLabel="イベントを主催する"
          hitSlop={10}
          ...
          <Entrance key={e.id} delay={i * 60}>
            ...
            <View style={styles.card}>
              <Pressable
                ...
                accessibilityRole="button"
                accessibilityLabel={`${e.title}を開く`}
                style={({ pressed }) => [styles.cardBody, pressed && { opacity: 0.92 }]}
              >
                <View style={styles.topRow}>
                  <Tag label={jp(JP_MEETUP, e.type)} />
                  <Text style={styles.when}>
                    {jpDateLabel(e.dateLabel)} · {jpTimeLabel(e.timeLabel)}
                  </Text>
                </View>
              ...
              <View style={styles.bottomRow}>
                <View style={styles.hostRow}>
                  ...
                  <Text style={styles.hostText} numberOfLines={1}>
                    {e.hostOwnerId === 'owner-1' ? 'あなたが主催' : `${e.hostName}さんが主催`}・{count}人参加
                  </Text>
                </View>
                <Pressable
                  onPress={() => rsvp(e.id, !going)}
                  accessibilityRole="button"
                  accessibilityLabel={going ? `${e.title}の参加を取り消す` : `${e.title}に参加する`}
